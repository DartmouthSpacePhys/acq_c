...
//	char syscom[200];
	...
	struct timeval now,then, acq_st, acq_et;
...
/*	if (geteuid() != 0) {
	...
	int maxsleeps = size/(datarate/100); // Assumes 10^4 us sleeps
	if (options.verbose) {
		printf("datarate: %d, bufsize: %lu, maximum sleeps: %d\n", datarate, size, maxsleeps);
	}
	int nsleeps = 0;
	int bcont;
	int usrvs = 0;
...
//				  printf("is: %llui, sm: %llui.\n",islices,options.slicemax);
	...
	} while (running && (nsleeps < maxsleeps));
	...
	gettimeofday(&acq_et, NULL);

	printf("bye now.  (Acquisition time: %d s)\n", acq_et.tv_sec-acq_st.tv_sec);
